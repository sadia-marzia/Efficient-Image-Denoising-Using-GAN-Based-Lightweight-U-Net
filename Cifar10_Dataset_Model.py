...
train_ds = tf.data.Dataset.from_tensor_slices((add_noise(x_train), x_train)).shuffle(1000).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
val_ds = tf.data.Dataset.from_tensor_slices((add_noise(x_val), x_val)).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
test_ds = tf.data.Dataset.from_tensor_slices((add_noise(x_test), x_test)).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
